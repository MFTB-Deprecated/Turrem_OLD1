...
public abstract class RenderIcon2D implements IRenderIcon
{
	...
	public void render(float xmin, float ymin, float xmax, float ymax)
	{
		this.render(xmin, ymin, xmax, ymax, 0, false, false);
	}
	
	
	public void render(float xmin, float ymin, float xmax, float ymax, int rotate, boolean flipx, boolean flipy)
	{
		this.start();
		GL11.glBegin(GL11.GL_TRIANGLES);
		for (int i = 0; i < 6; i++)
		{
			this.renderVert(xmin, ymin, xmax, ymax, i, rotate, flipx, flipy);
		}
		GL11.glEnd();
		this.end();
	}
	
	private void renderVert(float xmin, float ymin, float xmax, float ymax, int num, int rotate, boolean flipx, boolean flipy)
	{
		int ind = inds[num];
